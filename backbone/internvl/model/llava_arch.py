...
from config.backbone.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from backbone.shared.multimodal.routing_mixin import RoutingVisionMixin

from PEFT.tuners import HiDeMOELoraModel
from collections import deque


class LlavaMetaModel(RoutingVisionMixin):

    def __init__(self, config):
        super(LlavaMetaModel, self).__init__(config)

        if hasattr(config, "mm_vision_tower"):
            load_separately = getattr(config, "load_vision_tower_separately", True)
            self.vision_tower = build_vision_tower(config, delay_load=load_separately)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if self.get_vision_tower() is None:
            ...
            load_separately = getattr(
                model_args,
                "load_vision_tower_separately",
                getattr(self.config, "load_vision_tower_separately", True),
            )
            if load_separately:
                vision_tower.load_model()
            else:
                vision_tower.mark_loaded_from_checkpoint()
        ...
        if pretrain_mm_mlp_adapter is not None:
            ...
            has_vit_pos_embedding = any("position_embedding" in key for key in mm_projector_weights.keys())
            if has_vit_pos_embedding and hasattr(self.vision_tower, "vision_tower"):
                self.vision_tower.vision_tower.embeddings.load_state_dict(
    ...
    def initialize_text_modules(self, model_args, fsdp=None):
        ...
        self.initialize_routing_vision_modules(model_args, fsdp=fsdp)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def get_routing_vision_tower(self):
        return self.get_model().get_routing_vision_tower()
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images):
        ...
        vision_tower = self.get_vision_tower()
        expected_patches = getattr(vision_tower, "num_patches", 576)
        if not (type(images) is list or images.ndim == 5):
            assert image_features.shape[1] == expected_patches, (
                f"Expected {expected_patches} visual tokens, got {image_features.shape[1]}"
            )
